...
function DisplayCountryCode(code) {
  return <div style={{fontSize: 25}}><span className={`flag-icon flag-icon-${countryCodeMapping2Char[code]?.toLowerCase() || "xx"}`}></span> {countryCodeMapping[code] || "Unknown"}</div>;
}
...
export default class AppUnitTest extends React.Component {
  ...
  render() {
    ...
    return (<div style={{background: "white"}}>
      ...
      {answers.map(a => a.HighestCountries.map(c => c.CountryNum).concat(a.LowestCountries.map(c => c.CountryNum)))
        .reduce((acc, val) => acc.concat(val.filter(v => !acc.includes(v))), []).map(c => DisplayCountryCode(c))}
